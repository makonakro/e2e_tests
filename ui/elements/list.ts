...
const SELECTORS = {
  listItemByExactMatch: (listItemMatch, identifier) => `${listItemMatch}:text-is("${identifier}")`,
  listItemByPartialMatch: (listItemMatch, identifier) => `${listItemMatch}:has-text("${identifier}")`,
}
...
export default class List extends BaseElement {
  ...
  async getListElement(identifier: string | number, exact = false) {
    ...
    return this.element.locator(exact ? SELECTORS.listItemByExactMatch(this.listItem, identifier)
    : SELECTORS.listItemByPartialMatch(this.listItem, identifier));
  }

  private async getListItem(identifier: string | number, exact?:boolean): Promise<Locator> {
